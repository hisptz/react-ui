...
export const getStartEndDate = (data: { [x: string]: any }) =>
  formatStartEndDate(
...
export const getPeriodFromFilter = (filter: string | any[]) => {
...
export const getPeriodNameFromFilter = (filter: string | any[]) => {
...
export const getAuthToken = (baseUrl: string) =>
  new Promise(async (resolve, reject) => {
    const token = await apiFetch(`${baseUrl}/tokens/google`).catch(() => reject(new Error(i18n.t("Cannot get authorization token for Google Earth Engine."))));
...
export const getPeriods = async (eeId: string) => {
  const { periodType } = getEarthEngineLayer(eeId);

  const getPeriod = ({ id, properties }: { id: string; properties: Record<string, any> }) => {
...
export const defaultFilters = ({ id, name, year }: { id: string; name: string; year: number }) => [
  {
    type: "eq",
...
export const addPropNames = (layer: { aggregationType: any; name: any; legend: any }, data: { [x: string]: any }) => {
